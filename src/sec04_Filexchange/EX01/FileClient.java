...
//    Socket socket = null;
    ...
    try (Socket socket = new Socket(InetAddress.getByAddress(new byte[]{(byte)192,(byte)168,(byte)55,(byte)217}),10000);
         DataInputStream dis = new DataInputStream(new BufferedInputStream(socket.getInputStream() ));
         DataOutputStream dos = new DataOutputStream(new BufferedOutputStream(socket.getOutputStream() ));
         BufferedInputStream bis = new BufferedInputStream(new FileInputStream(infile));) {

      System.out.println("  server conntion complete  ");
      System.out.println(socket.getInetAddress()+"."+socket.getPort()+"   "+socket.getLocalSocketAddress());
